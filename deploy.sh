...
# to skip all the setup step and just redeploy the app with the existing configuration, you can run the script with the -update flag:
#   ./deploy.sh -update
UPDATE_ONLY=false
if [[ "$1" == "-update" ]]; then
    UPDATE_ONLY=true
    echo "Update only mode"
fi

if [[ "$UPDATE_ONLY" == "false" ]]; then
    echo "Running full deployment with setup"
...
cd ..
rm -rf deploy
echo "Deployment complete! Your app should be live at https://$APP_SERVICE_NAME.azurewebsites.net"